...
int n, a[3][100005], f[3][100005], ans = 0;
...
void solve(){
    ...
    for(int i = 2; i <= n; ++i){
        if(i == 2){
            ...
            f[1][i] = max(f[2][i-1], max(f[1][i-2], f[2][i-2])) + a[1][i];
            f[2][i] = max(f[1][i-1], max(f[1][i-2], f[2][i-2])) + a[2][i];
        }
        ans = max(ans, max(f[1][i], f[2][i]));
    }
    cout << ans;
}
